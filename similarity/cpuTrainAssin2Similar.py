...
# running this script you train our model starting from Bert Base Multilingue


# You need to download the initial model files: https://storage.googleapis.com/bert_models/2018_11_23/multi_cased_L-12_H-768_A-12.zip
# -bertmodel files(they go into the bertDir):
#              bertmodel.ckpt.data
#              bertmodel.ckpt.index
#              bertmodel.ckpt.meta
#              vocab.txt
#              bert_config.json
...
# submission 1:#################################################
#
# bertDir -> the unziped file that contains all the necesary files in this dir
...
flags.DEFINE_string("task_name", "assin2similarity", "The name of the task to train.")
...
class InputExample(object):
 ...
 def __repr__(self):
     return '[' + self.guid + '||' + self.text_a + '||' +   self.text_b + '||'+str(self.label) + ']'
...
class assin2SimilarityProcessor(DataProcessor):
...
def findMaxLength(examples):
    """ examples is a list of 'InputExample' objects
    this function returns the maximum of the sum of the number of tokens for
    text_a and text_b
    """
    maxim = 0
    ...
    for ex in examples:
        tokens1 = tokenizerFromHub.tokenize(ex.text_a)
        tokens2 = tokenizerFromHub.tokenize(ex.text_b)
        aux = len(tokens1) + len(tokens2)
        if aux > maxim:
            maxim = aux

    return maxim # on assin1 train set result is 87
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      def metric_fn(label_ids, logits, is_real_example):
        ...
        pearson = tf.contrib.metrics.streaming_pearson_correlation(logits,
                                                                   label_ids)

        return {
            "eval_pearson_correlation": pearson,
            "eval_loss(mse)": mse,
        }
...
def main(_):
  ...
  processors = {
      "assin2similarity": assin2SimilarityProcessor,
  }
  ...
  if FLAGS.do_eval:
    ...
    if restoreFile == None:
      ...
      result = estimator.evaluate(input_fn=eval_input_fn, steps=eval_steps, checkpoint_path=restoreFile)
  ...
  if FLAGS.do_predict:
    ...
    with tf.gfile.GFile(output_predict_file, "w") as writer:
      ...
      for (i, prediction) in enumerate(result):
        ...
        output_line = str(logits) + '\n'    
        writer.write(output_line)
        num_written_lines += 1

    #### DEBUG    
    if num_written_lines != num_actual_predict_examples:
      print("num_written_lines="+ str(num_written_lines))
      print("num_actual_predict_examples=" + str(num_actual_predict_examples))   
